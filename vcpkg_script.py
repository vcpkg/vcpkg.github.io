...
# Finding all ports in the ports folder
def getFiles(path):
...
# finidng all supported triplets from ci.baseline.txt
def gen_ci():
...
# generate the json for all files by looking at control or manifest files
def gen_all_files():
